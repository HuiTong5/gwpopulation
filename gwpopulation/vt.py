...
class mixture_two_ResamplingVT(_BaseVT):
    """
    Evaluate the sensitive volume using a set of found injections.
    ...
    def __init__(
        ...
        super(mixture_two_ResamplingVT, self).__init__(model=model, data=data)
        self.n_events = n_events
        self.total_injections = data.get("total_generated", len(data["prior"]))
        self.analysis_time = data.get("analysis_time", 1)
        self.redshift_model = None
    ...
    def detection_efficiency(self, parameters):
        ...
        weights = self.parameters["likelihood_mix_mass"] * self.parameters["likelihood_mix_spin"]*weights1 + (
        self.parameters["likelihood_mix_mass"] * (1-self.parameters["likelihood_mix_spin"])*weights2) + (
        (1 - self.parameters["likelihood_mix_mass"]) * self.parameters["likelihood_mix_spin"]*weights3) + (
        (1 - self.parameters["likelihood_mix_mass"]) * (1-self.parameters["likelihood_mix_spin"])*weights4)
        mu = float(xp.sum(weights) / self.total_injections)
        var = float(
            xp.sum(weights**2) / self.total_injections**2
            - mu**2 / self.total_injections
